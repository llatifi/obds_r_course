...
```{r}
iris_features <- subset(iris, select = c(Sepal.Length, Sepal.Width, Petal.Length, Petal.Width))
head(iris_features)

# Can also use indexing:
other_iris_feature <- iris[1:4]
head(other_iris_feature)
```
...
plot(pca_iris, col = 2:5)
plot(pca_iris, col = c(446, 541, "yellow", "green"))
```
...
```{r}
explained_variance_ratio <- summary(pca_iris)
explained_variance_ratio
str(explained_variance_ratio)

test_explained_variance_ratio <- explained_variance_ratio$importance[2, ]
test_explained_variance_ratio

# Solution
solution_explained_variance_ratio <- (pca_iris$dev ^ 2 / sum(pca_iris$dev ^ 2))
solution_explained_variance_ratio
...
```{r}
?barplot()
barplot(test_explained_variance_ratio, main = "Iris Percentage of Variance Explained", col = 5:8)
```
...
plot(
  x = pca_iris$x[ , "PC1"], 
  y = pca_iris$x[ , "PC2"], 
  col = iris_hclust_ward_labels, 
  main = "Iris Ward Clustering", 
  xlab = "Iris PC1", 
  ylab = "Iris PC2"
  )
plot(
  x = pca_iris$x[ , "PC1"], 
  y = pca_iris$x[ , "PC2"], 
  col = iris_hclust_complete_labels,
  main = "Iris Complete Clustering", 
  xlab = "Iris PC1", 
  ylab = "Iris PC2"
  )
plot(
